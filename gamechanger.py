...
# Define the standard stats columns expected in the box score data
stats_cols_excel_headers = ['MIN', 'PTS', 'FG', 'FG_PCT', '3PT', '3P%', 'FTA', 'FT%', 'OREB', 'DREB', 'REB', 'AST', 'STL', 'BLK', 'TOV', 'PF']
...
# REMOVED: box_score_sheet_names_expected will now be replaced by regex matching
# Define the list of actual box score sheet names (used for filtering in process_box_scores)
# This is defined once globally to avoid scope issues.
# box_score_sheet_names_expected = [
#     "Blue vs White",
#     "White vs Beige",
#     "Black vs White",
#     "Blue Vs Grey", # Note: This one uses "Vs"
#     "Black vs Beige", # Added for comprehensive sheet processing
#     "Blue vs Beige",  # Added for comprehensive sheet processing
#     "White vs Grey",  # Added for comprehensive sheet processing
#     "Blue vs Black",  # CRITICAL ADDITION: Based on user's specific query about Player 13
#     "Beige vs White", # Added for comprehensive sheet processing
# ]

# Regex pattern for box score sheet names (e.g., "Team1 vs Team2")
# This pattern will match any two words separated by " vs " or " Vs "
...
def process_box_scores():
    ...
    def clean_single_box_score_sheet(df_sheet_raw, sheet_name, stats_cols_headers, team_labels):
        """
        ...
        This is the original logic from the notebook's Cell 3.
        """
        df = df_sheet_raw.copy() # Work on a copy of the raw sheet data

        if df.empty:
            print(f"⚠️ Sheet '{sheet_name}' is empty, skipping.")
            return None

        # --- Step 1: Identify Header Row and all Team Headers ---
        # `expected_player_data_headers` are what defines the start of player stats.
        expected_player_data_headers = ["Player No.", "PTS", "REB", "AST", "FG", "STL", "BLK", "TOV", "PF"] # Simplified for detection

        header_row_idx = None
        team_header_locations = [] # Stores (row_idx, 'Team Name')
        base_team_names_lower = {t.lower(): t.title() for t in team_labels}

        for i in range(len(df)):
            row_values = df.iloc[i].astype(str).str.lower().values
            first_col_value_raw = df.iloc[i, 0] # First cell of the row
            normalized_first_col_val = str(first_col_value_raw).strip().lower()

            # Check for player data header row
            if header_row_idx is None and any(col.lower() in val for val in row_values for col in expected_player_data_headers):
                header_row_idx = i
                # Once header is found, player data starts from next row.
                # Team headers can still exist below this, so continue.

            # Check for explicit team headers in the first column
            if normalized_first_col_val in base_team_names_lower:
                ...
                # print(f"  Debug: Found team header '{base_team_names_lower[normalized_first_col_val]}' at row {i} in sheet '{sheet_name}'.") # Debugging print

        if header_row_idx is None:
            print(f"⚠️ No valid data header found in '{sheet_name}'. Skipping as a non-box score sheet.")
            return None

        # --- Step 2: Extract Player Data and Assign Teams ---
        processed_player_records = []
        current_team_for_players = None

        # Get the actual headers from the detected header row
        original_headers_for_cols = df_sheet_raw.iloc[header_row_idx].values 
        
        # Safely handle column names, ensuring uniqueness
        new_columns = []
        seen_columns = {}
        for col in original_headers_for_cols:
            col_str = str(col).strip()
            if col_str == "nan" or col_str == "":
                col_str = f"Unnamed: {len(new_columns)}"
        ...
        # Iterate from the row *after* the header row to the end of the sheet
        for i in range(header_row_idx + 1, len(df_sheet_raw)):
            row_data = df_sheet_raw.iloc[i].copy() # Get the raw row data

            # Check if this row is a team header itself
            first_col_value_raw = row_data.iloc[0]
            normalized_first_col_val = str(first_col_value_raw).strip().lower()
            if normalized_first_col_val in base_team_names_lower:
                current_team_for_players = base_team_names_lower[normalized_first_col_val]
                continue # Skip this row as it's a new team header

            # Update current_team_for_players based on team headers found *before* this row
            # Find the most recent team header before the current row 'i'
            relevant_team_headers = [name for idx, name in team_header_locations if idx < i]
            if relevant_team_headers:
                current_team_for_players = relevant_team_headers[-1] # Take the most recent one

            # Attempt to clean and standardize 'Player No.'
            # Create a temporary Series with the new columns for easy `.get()` access
            temp_row_series = pd.Series(row_data.values, index=new_columns)

            player_no_raw = temp_row_series.get('Player No.', np.nan)
            ...
            # Only process if we have a valid player number AND a current team assigned
            if pd.notna(player_no_cleaned) and current_team_for_players is not None:
                ...
                for col in stats_cols_headers:
                    val = temp_row_series.get(col, np.nan)
                    numeric_val = 0 # Default
                    ...
                    if pd.isna(val) or str(val).strip().lower() in ['dnp', '-', ' ']:
                        ...
                        # --- CRITICAL FIX: Handle 'X-Y' format for FG, 3PT, FTA ---
                        if col in ['FG', '3PT', 'FTA']:
                            match_xy = re.match(r'(\d+)-(\d+)', val_str)
                            if match_xy:
                                numeric_val = int(match_xy.group(1)) # Take 'X' as made shots
                            else:
                                numeric_val = pd.to_numeric(val_str, errors='coerce')
                                numeric_val = numeric_val if pd.notna(numeric_val) else 0
                        # --- CRITICAL FIX: Handle percentages (e.g., '57%', '90.00%') ---
                        elif col in ['FG_PCT', '3P%', 'FT%']:
                            if '%' in val_str:
                    ...
                    player_record[col] = numeric_val

                    if numeric_val != 0:
                        all_stats_are_zero = False

                if not all_stats_are_zero:
                    processed_player_records.append(player_record)
            elif pd.notna(player_no_cleaned) and current_team_for_players is None:
                pass # Suppress this print for clean output unless critical


        if processed_player_records:
            df_cleaned = pd.DataFrame(processed_player_records)
        else:
            df_cleaned = pd.DataFrame(columns=['Player No.', 'Team', 'Game'] + stats_cols_headers)

        # Final cleanup and type conversions
        for col in stats_cols_headers:
            if col in df_cleaned.columns:
                if col == 'MIN': # Re-handle DNP in case it somehow slipped through
                    df_cleaned[col] = df_cleaned[col].replace('DNP', 0)
                # Ensure values are numeric, filling NaN from previous parsing with 0
                df_cleaned[col] = pd.to_numeric(df_cleaned[col], errors='coerce').fillna(0)
            else:
                df_cleaned[col] = 0 # Add missing stat columns as 0

        # Ensure Player No. and Team are clean
        if 'Player No.' in df_cleaned.columns:
            df_cleaned['Player No.'] = pd.to_numeric(df_cleaned['Player No.'], errors='coerce').fillna(0).astype(int)
        if 'Team' in df_cleaned.columns:
            df_cleaned['Team'] = df_cleaned['Team'].astype(str).str.strip().replace('nan', np.nan) # Clean up 'nan' string
            df_cleaned.dropna(subset=['Team'], inplace=True) # Drop if team is still NaN after cleaning

        # Drop rows where all statistical columns are zero (likely summary rows or empty player entries)
        # Exclude 'Player No.', 'Team', 'Game' from this check
        stats_cols_in_df = [col for col in stats_cols_headers if col in df_cleaned.columns]
        if not df_cleaned.empty and stats_cols_in_df:
            df_cleaned = df_cleaned[df_cleaned[stats_cols_in_df].sum(axis=1) != 0].copy()
        else:
            # If after player filtering, the df is empty or no stats columns, return None
            if df.empty:
                print(f"Debug: DataFrame for '{sheet_name}' is empty after player filtering. Returning None.")
            else:
                print(f"Debug: No stats columns found in '{sheet_name}' after filtering. Returning None.")
    ...
    # ✅ Process all sheets dynamically from the loaded Excel file
    cleaned_dfs = {}
    for sheet_name in xls.sheet_names:
        try:
            ...
            # NEW: Filter for expected box score sheets using regex pattern
            ...
            # Pass the original sheet_name to the clean_single_box_score_sheet
            cleaned_df = clean_single_box_score_sheet(pd.read_excel(xls, sheet_name=sheet_name), sheet_name, stats_cols_excel_headers, team_labels)
...
def generate_summary_statistics():
    ...
    if df_combined.empty:
        ...
        numeric_cols_for_mean = [col for col in stats_cols_excel_headers + ['Efficiency'] if col in df_combined.columns]
...
def prepare_visual_data():
    ...
    if df_combined.empty:
        ...
        print(df_visual[['Player No.', 'Team', 'Game', 'PTS', 'REB', 'AST', 'Player_Team_Label', 'Efficiency', 'PTS_Per_Min', 'TS%']].head()) # Include some advanced stats to verify
...
def generate_advanced_visualizations():
    ...
    if df_visual.empty:
        ...
        required_plot_cols = ['PTS', 'REB', 'AST', 'Efficiency', 'Player_Team_Label', 'Game', 'FG_PCT', 'TOV']
...
def calculate_advanced_stats(): 
    ...
    if df_combined.empty:
        ...
        # Handle division by zero: if TOV is 0, the ratio is undefined (can be seen as very good).
        # Replace 0 turnovers with np.nan for division, then fill infinite/NaN values with a large number or 0.
        df_advanced_stats['AST_TOV_Ratio'] = (df_advanced_stats['AST'] / df_advanced_stats['TOV'].replace(0, np.nan)).fillna(0)
        # For players with AST > 0 and TOV = 0 specifically, assign a very high ratio (e.g., 999)
        df_advanced_stats.loc[(df_advanced_stats['AST'] > 0) & (df_advanced_stats['TOV'] == 0), 'AST_TOV_Ratio'] = 999

        print("✅ Assist-to-Turnover Ratio calculated.")

        # --- True Shooting Percentage (TS%) ---
        # CHALLENGE: Requires FGA (Field Goal Attempts) and 3PA (3-Point Attempts).
        # Our `stats_cols_excel_headers` only contains `FG` (Made), `FG_PCT`, `3PT` (Made), `3P%`.
        # We must infer FGA and 3PA, which can be problematic if percentages are 0.
        print("⚠️ Note on TS%: FGA and 3PA are inferred from FG_PCT and 3P% in the source data.")
        print("   This can be less accurate than direct FGA/3PA fields if percentages are unreliable (e.g., 0).")
        
        # Adjust percentages to avoid division by zero: replace 0% with NaN to get Inf/NaN, then handle.
        # Add a tiny epsilon to avoid direct 0 division if FG_PCT/3P% are exactly 0, but this can skew data.
        # It's better to let them be NaN, and then fill based on the corresponding made shots.
        df_advanced_stats['FG_PCT_adjusted_for_div'] = df_advanced_stats['FG_PCT'].replace(0, np.nan)
        df_advanced_stats['3P%_adjusted_for_div'] = df_advanced_stats['3P%'].replace(0, np.nan)

        # Infer FGA and 3PA
        df_advanced_stats['FGA_inferred'] = df_advanced_stats['FG'] / df_advanced_stats['FG_PCT_adjusted_for_div']
        df_advanced_stats['3PA_inferred'] = df_advanced_stats['3PT'] / df_advanced_stats['3P%_adjusted_for_div']

        # If made shots are 0, attempts must be 0 (regardless of percentage).
        # If made shots are >0 but percentage is NaN (was 0), attempts could be Inf, so fill with reasonable number
        # A common approach is to just use the made shots if percentage is zero.
        # Let's ensure attempts are 0 if makes are 0.
        df_advanced_stats.loc[df_advanced_stats['FG'] == 0, 'FGA_inferred'] = 0
        df_advanced_stats.loc[df_advanced_stats['3PT'] == 0, '3PA_inferred'] = 0

        # Fill any remaining NaNs (e.g., if a player had non-zero FG_PCT but no FG) with 0
        df_advanced_stats['FGA_inferred'] = df_advanced_stats['FGA_inferred'].fillna(0)
        df_advanced_stats['3PA_inferred'] = df_advanced_stats['3PA_inferred'].fillna(0)

        # Calculate Total Shot Attempts for TS% denominator: FGA + 0.44 * FTA
        df_advanced_stats['TS_Denominator'] = df_advanced_stats['FGA_inferred'] + 0.44 * df_advanced_stats['FTA']
        ...
        df_advanced_stats.drop(columns=[
            'MIN_adjusted', 'FG_PCT_adjusted_for_div', '3P%_adjusted_for_div',
            'FGA_inferred', '3PA_inferred', 'TS_Denominator', 'Total_Game_REB'
        ...
        # Corrected: Changed 'PPM' to 'PTS_Per_Min'
...
def generate_advanced_summary_statistics(): 
    ...
    if df_combined.empty:
        ...
        player_advanced_stats_avg = df_combined.groupby('Player_Team_Label').agg(
            ...
            Avg_Efficiency=('Efficiency', 'mean'), # Corrected: Added Avg_Efficiency here
            Avg_WS_Simplified=('WS_Simplified', 'mean'), # Added WS_Simplified
            Avg_VORP_Simplified=('VORP_Simplified', 'mean'), # Added VORP_Simplified
